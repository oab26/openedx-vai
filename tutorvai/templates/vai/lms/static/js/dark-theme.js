$(document).ready(function() {
    ...
    const themeCookie = 'vai-toggle-dark';

    function applyThemeOnPage(){
      const theme = $.cookie(themeCookie);
      {% if VAI_ENABLE_DARK_TOGGLE %}
      $('body').toggleClass("vai-dark-theme", theme === 'dark');       // append or remove dark-class based on cookie-value
      // update expiry
    ...
    window.addEventListener("message", function(e){
      if (e.data && e.data["vai-toggle-dark"]){
        applyThemeOnPage();
      }
    });
